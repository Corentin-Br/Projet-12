...
class MyTest(TestCase):
    ...
    def test_gestion_user_can_access_a_specific_user(self):
        self.client.login(**self.gestion_logs)
        for pk in range(1, 4):
            resp = self.client.get(f"/admin/accounts/myuser/{pk}/change/")
            assert resp.status_code == 200
    ...
    def test_gestion_user_can_modify_a_specific_user(self):
        self.client.login(**self.gestion_logs)
        resp = self.client.post("/admin/accounts/myuser/1/change/", {"first_name": "Jacques", "email": self.gestion_user.email, "last_name": self.gestion_user.last_name, "role": self.gestion_user.role, "is_active": self.gestion_user.is_active})
    ...
    def test_support_and_sales_user_cant_modify_a_specific_user(self):
        for logs in (self.support_logs, self.sales_logs):
            self.client.login(**logs)
            resp = self.client.post("/admin/accounts/myuser/1/change/",
